...
export function OperationsIntelligencePanel({ model }: { model: IssueConsoleModel | null }): JSX.Element {
  ...
  const reliability = useMemo(() => {
    ...
    return {
      ...
      risks: items.filter(item => ['watch', 'breach'].includes(item.sourceReliability?.slo_state || '')).sort((a, b) => (a.sourceReliability?.live_percent || 0) - (b.sourceReliability?.live_percent || 0)).slice(0, 8),
      canaries: items.filter(item => item.schemaCanary?.state === 'changed').slice(0, 8)
    };
  }, [model]);
  ...
  return (
    <>
      ...
      {open && <aside id="ops-intelligence-panel" className="ops-intel-panel" aria-label="Operations intelligence">
        ...
        <p className="ops-intel-boundary">Collector reliability and parser shape are evidence controls, not vendor service health. Event clusters are temporal correlations only and never causal claims.</p>
        <section>
          <h3>Seven-day source observation SLO</h3>
          <div className="ops-intel-metrics">
            <div><b>{reliability.meeting}</b><span>Meeting</span></div>
            <div><b>{reliability.watch}</b><span>Watch</span></div>
            <div><b>{reliability.breach}</b><span>Breach</span></div>
            <div><b>{reliability.warming}</b><span>Warming</span></div>
          </div>
          {reliability.risks.length ? <div className="ops-intel-list">{reliability.risks.map(item => <article key={item.id}>
            ...
            <p>{item.sourceReliability?.live_percent}% live observations across {item.sourceReliability?.sample_count} samples. {item.sourceReliability?.schema_change_count || 0} schema changes.</p>
        ...
        <section>
          <h3>Parser canaries</h3>
          {reliability.canaries.length ? <div className="ops-intel-list">{reliability.canaries.map(item => <article key={item.id}>
            <div><b>{item.provider}</b><span>Shape changed</span></div>
            <p>Parser {item.parserVersion || 'unknown'} accepted a new source fingerprint at {timeLabel(item.schemaCanary?.last_changed_at || item.checkedAt)}. Review source semantics independently of service state.</p>
          </article>)}</div> : <p className="ops-intel-empty">No current source-shape canary is reporting a change.</p>}
        </section>
        <section>
