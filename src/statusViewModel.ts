...
export function wallboardProviderPriority(provider: DiagnosticSource): number {
    if (provider.serviceState === 'major' && provider.attention === 'critical') return 1000;
    if (provider.serviceState === 'degraded' && provider.attention === 'action') return 900;
    if (provider.changeTypes.includes('incident_new')) return 850;
    if (provider.changeTypes.includes('severity_increased')) return 840;
    if (provider.changeTypes.includes('source_unavailable') && provider.criticality === 'high') return 800;
    if (provider.sourceState === 'unavailable') return 700;
    if (provider.changeTypes.some(type => ['incident_resolved', 'service_recovered', 'source_recovered'].includes(type))) return 600;
    if (provider.sourceState === 'limited') return 500;
    if (provider.serviceState === 'unknown') return 400;
    return 100;
}
