...
test('browser validation requires reconciled reliability history and canary metadata', () => {
  const missing = structuredClone(p);
  delete missing.providers[0].source_reliability;
  delete missing.providers[0].schema_canary;
  assert.ok(payloadValidationErrors(missing).some(error => error.includes('missing source_reliability')));
  assert.ok(payloadValidationErrors(missing).some(error => error.includes('missing schema_canary')));
