...
export function IssueConsole({ model, lifecycle, onRefresh }: { model: IssueConsoleModel | null; lifecycle: DataLifecycle; onRefresh: () => void }): JSX.Element {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (typing) return;
      if (event.key === '/') { event.preventDefault(); navigate('providers'); window.setTimeout(() => searchRef.current?.focus(), 0); }
  ...
  return (
    <div className="command-shell">
      ...
      {view === 'providers' && <input ref={searchRef} className="sr-only" aria-hidden tabIndex={-1} />}
    </div>
  );
}
