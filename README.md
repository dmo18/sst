...
The canonical payload is deliberately generated by GitHub Actions; the browser overlay is an enhancement, not a replacement for collection. The full provider, CORS, legacy-display, fallback, and hard-freshness analysis is in [browser collection feasibility](docs/browser-collection-feasibility.md).
...
- `.github/workflows/status-freshness-watch.yml` checks deployed payload age and dispatches one refresh when the payload is older than 10 minutes or official structured truth drifts while no release is active.
- `.github/workflows/refresh-continuity.yml` starts only after a successful scheduled refresh. It bridges up to three missed five-minute scheduler windows with serialized GitHub-native recovery dispatches, then stops. A later successful schedule cancels and replaces the older bridge.
