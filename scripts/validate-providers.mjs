...
if (!Array.isArray(catalog)) {
  ...
  for (const [index, provider] of catalog.entries()) {
    ...
    if (typeof provider.priority !== 'undefined' && (!Number.isInteger(provider.priority) || provider.priority < 0)) errors.push(fail('priority must be a non-negative integer when present.', context));
    ...
    if (limitedSourceTypes.has(provider.sourceType) && (typeof provider.message !== 'string' || !provider.message.trim())) errors.push(fail('Limited source types require a limitation message.', context));
    if (provider.category) categories.add(provider.category);
  }
}

for (const [url, providers] of sharedUrls) if (providers.length > 1) console.warn(`Shared official source: ${providers.join(', ')} -> ${url}`);
